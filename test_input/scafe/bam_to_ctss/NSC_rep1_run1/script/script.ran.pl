...
=head1 SYNOPSIS
 ...
 Usage:
   ...
   --max_softclip_length   (optional) [integer] maximum length of allowed softclip at 5'end of the read to be
   ...
   --unencoded_G_upstrm_nt (optional) [integer] the number of nucleotide used to cound the nunber of 
...
v1.0.1 [July 5, 2023]
	-added TSS_mode option to allow softclip defined TSS
	[June 26, 2022]
	-Modified bamtobed command to accommodate long reads
	-Fixed out of bound error in long read
	
	[November 26, 2023 4:47 PM]
	-added max_softclip_length option, also made num_nt_extract_check depdent on max_softclip_length as num_nt_extract_check = max_softclip_length + 10
	-added unencoded_G_upstrm_nt option, which will restrict the regions to count unencoded G at number of nt upstream of the end5 
...
my ($curntTimeStamp) = &timeStamp();#->1390
my $ARGVStr = join "\n", (&currentTime(), $scriptAbsPath, @ARGV);#->495
...
my ($bamPath, $exclude_flag, $include_flag, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt, $min_MAPQ, $max_thread, $genome, $outputPrefix, $outDir, $overwrite) = &readParameters();#->1311
#<\section>
#====================================================================================================================================================#
...
&logCalledCMDAndScript($ARGVStr, $result_script_dir, $scriptAbsPath);#->840
&printStartOrFinishMessage("startMessage");#->1183
&reportAndLogStatus("Main settings are:", 10, "\n");#->1368
&reportAndLogStatus("exclude_flag=$exclude_flag", 10, "\n");#->1368
&reportAndLogStatus("TSS_mode=$TSS_mode", 10, "\n");#->1368
&reportAndLogStatus("max_softclip_length=$max_softclip_length", 10, "\n");#->1368
&reportAndLogStatus("unencoded_G_upstrm_nt=$unencoded_G_upstrm_nt", 10, "\n");#->1368
&reportAndLogStatus("min_MAPQ=$min_MAPQ", 10, "\n");#->1368
my ($tabix_bin, $bgzip_bin, $bedtools_bin, $samtools_bin, $paraclu_bin, $cut_sh_path, $bedGraphToBigWig_bin, $bigWigAverageOverBed_bin) = &checkAllExecutable();#->238
my ($chrom_name_path, $chrom_size_path, $genome_fasta_path) = &checkChromNameSizeGenomeFastaPath($genome);#->338
#<\section>
#====================================================================================================================================================#
...
my ($chrom_name_size_hsh_ref, $out_chrom_size_hsh_ref) = &generateChromNameConversionHsh($chrom_name_path, $chrom_size_path, $samtools_bin, $bamPath);#->513
my ($chunk_size) = &generateChunkSize($out_chrom_size_hsh_ref, $num_chunk);#->634
my ($chunk_info_hsh_ref) = &generateChunkInfoHsh($chrom_name_size_hsh_ref, $chunk_size, $result_tmp_dir);#->583
&processPerChunk($bamPath, $bedtools_bin, $samtools_bin, $max_thread, $chunk_info_hsh_ref, $chrom_name_size_hsh_ref, $exclude_flag, $include_flag, $min_MAPQ, $genome_fasta_path, $out_chrom_size_hsh_ref, $sanity_check_toggle, $min_match_length, $max_non_G_mismatch, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt);#->1217
&poolChunkResults($chunk_info_hsh_ref, $result_bed_dir, $result_log_dir, $paramTag, $max_thread, $tabix_bin, $bgzip_bin, $result_tmp_dir, $rm_tmp_file);#->945
#<\section>
#====================================================================================================================================================#
...
&printOutputFileListAndReadme($ARGVStr, $paramTag, $outDir);#->1068
&printStartOrFinishMessage("finishMessage");#->1183
#<\section>
#====================================================================================================================================================#
...
#	dependOnSub: reportAndLogStatus|1368
#	appearInSub: >none
#	primaryAppearInSection: 2_defineOutDirPath|159
...
#	subroutineCategory: unassigned
#	dependOnSub: reportAndLogStatus|1368
#	appearInSub: poolChunkResults|945, processPerChunk|1217
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 3_convert|186
...
#	dependOnSub: getCTSS|659, reportAndLogStatus|1368
#	appearInSub: processPerChunk|1217
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 3_convert|186
#	input: $TSS_mode, $bamPath, $bedtools_bin, $chrom_name_size_hsh_ref, $chunk_endPos, $chunk_fasta_offset_pos, $chunk_fasta_path, $chunk_log_path, $chunk_long_ctss_bed_path, $chunk_sanity_check_path, $chunk_seq_str, $chunk_startPos, $chunk_unsort_long_ctss_bed_path, $exclude_flag, $include_flag, $max_non_G_mismatch, $max_softclip_length, $min_MAPQ, $min_match_length, $region_str, $samtools_bin, $sanity_check_toggle, $unencoded_G_upstrm_nt
#	output: 
#	toCall: &convertBamToCTSS($bamPath, $chrom_name_size_hsh_ref, $exclude_flag, $include_flag, $bedtools_bin, $samtools_bin, $min_MAPQ, $chunk_long_ctss_bed_path, $chunk_unsort_long_ctss_bed_path, $region_str, $chunk_log_path, $chunk_startPos, $chunk_endPos, $chunk_fasta_path, $chunk_seq_str, $chunk_fasta_offset_pos, $chunk_sanity_check_path, $sanity_check_toggle, $min_match_length, $max_non_G_mismatch, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt);
#	calledInLine: 1275
#....................................................................................................................................................#
	my ($bamPath, $chrom_name_size_hsh_ref, $exclude_flag, $include_flag, $bedtools_bin, $samtools_bin, $min_MAPQ, $chunk_long_ctss_bed_path, $chunk_unsort_long_ctss_bed_path, $region_str, $chunk_log_path, $chunk_startPos, $chunk_endPos, $chunk_fasta_path, $chunk_seq_str, $chunk_fasta_offset_pos, $chunk_sanity_check_path, $sanity_check_toggle, $min_match_length, $max_non_G_mismatch, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt) = @_;
	...
	if (not -s $chunk_long_ctss_bed_path or $force_rerun eq 'yes') {
		...
		while (my $line = <BAMIN>) {
			...
			#&reportAndLogStatus("passed_read_num = $passed_read_num", 10, "\n") if ($passed_read_num%10000 == 0);#->1368
			my ($end5Start, $unencoded_G, $sanity_check_ary_ref) = &getCTSS($bedStart, $bedEnd, $strand, $bam_seq, $chunk_startPos, $chunk_endPos, $chunk_seq_str, $chunk_fasta_offset_pos, $samtools_bin, $out_chrom, $cigar, $min_softclip_length, $max_softclip_length, $min_match_length, $size, $TSS_mode, $unencoded_G_upstrm_nt);#->659
...
#	dependOnSub: >none
#	appearInSub: printStartOrFinishMessage|1183, reportAndLogStatus|1368
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 2_defineOutDirPath|159, 4_finishingTasks|198
#	input: none
#	output: $runTime
#	toCall: my ($runTime) = &currentTime();
#	calledInLine: 126, 1199, 1203, 1208, 1212, 1384, 1385
#....................................................................................................................................................#
...
#	dependOnSub: reportAndLogStatus|1368
#	appearInSub: >none
#	primaryAppearInSection: 3_convert|186
#	secondaryAppearInSection: >none
...
#	subroutineCategory: unassigned
#	dependOnSub: reportAndLogStatus|1368
#	appearInSub: convertBamToCTSS|385
#	primaryAppearInSection: >none
#	secondaryAppearInSection: >none
#	input: $TSS_mode, $bam_seq, $bedEnd, $bedStart, $chunk_endPos, $chunk_fasta_offset_pos, $chunk_seq_str, $chunk_startPos, $cigar, $max_softclip_length, $min_match_length, $min_softclip_length, $out_chrom, $samtools_bin, $size, $strand, $unencoded_G_upstrm_nt
#	output: $end5Start, $num_unencoded_G, $sanity_check_ary_ref
#	toCall: my ($end5Start, $num_unencoded_G, $sanity_check_ary_ref) = &getCTSS($bedStart, $bedEnd, $strand, $bam_seq, $chunk_startPos, $chunk_endPos, $chunk_seq_str, $chunk_fasta_offset_pos, $samtools_bin, $out_chrom, $cigar, $min_softclip_length, $max_softclip_length, $min_match_length, $size, $TSS_mode, $unencoded_G_upstrm_nt);
#	calledInLine: 460
#....................................................................................................................................................#
	my ($bedStart, $bedEnd, $strand, $bam_seq, $chunk_startPos, $chunk_endPos, $chunk_seq_str, $chunk_fasta_offset_pos, $samtools_bin, $out_chrom, $cigar, $min_softclip_length, $max_softclip_length, $min_match_length, $size, $TSS_mode, $unencoded_G_upstrm_nt) = @_;
	
	my ($end5Start, $num_unencoded_G);
	my $sanity_check_ary_ref = [];
	my $num_nt_extract_check = $max_softclip_length + 5;
	my $max_non_G_mismatch = $max_softclip_length - 1;
	...
	if ($softclip_valid eq 'yes' and $match_valid eq 'yes' and $TSS_within_chunk eq 'yes') {
		...
		my $softclip_seq_rev = reverse(substr $rd_seq, $check_read_pos, $softclip_length);
		my $trim_length = 0;

		my $unencoded_G_pos_hsh_ref = {};
		my $matched = 'no';
		my $non_G_mismatch = 0;
		foreach my $shift_pos (0..$num_nt_extract_check) {
			...
			if ($read_nt ne $genome_nt) {
				if ($read_nt eq 'G') {
					$unencoded_G_pos_hsh_ref->{$shift_pos}++;
				} else {
					$non_G_mismatch++;
				};
				$trim_length++;
				last if ($non_G_mismatch > $max_non_G_mismatch) and $TSS_mode eq 'first_match';
			} else {
				$matched = 'yes';
				last if $TSS_mode eq 'first_match';
			}
			if ($shift_pos == $softclip_length and $TSS_mode eq 'softclip') {
				$trim_length = $softclip_length;
				last;
			};
		}
		...
		if ($matched eq 'yes') {
			...
			#---[2023/11/26 20:00] count unencoded G 
			#$num_unencoded_G = keys %{$unencoded_G_pos_hsh_ref};

			$num_unencoded_G = 0;
			foreach my $shift_pos (keys %{$unencoded_G_pos_hsh_ref}) {
				if (($trim_length - $shift_pos) <=  $unencoded_G_upstrm_nt) {#---[2023/11/26 21:11] 0 based, for example, 0 and 1 for the first two upstream nt
					$num_unencoded_G++;
				}
			}
			...
			#---[2022/07/27 1:40] to prevent out of bound error in long read
			if ($end5Start >= $size) {
				$end5Start = undef;
				$num_unencoded_G = undef;
				$sanity_check_ary_ref=[];
			} elsif ($end5Start < 0) {
				$end5Start = 0 ;
			}

			$sanity_check_ary_ref = [$out_chrom, $bedStart, $bedEnd, $strand, $rd_seq, $check_read_seq, $check_genome_seq, $cigar, $softclip_length, $softclip_seq_rev, $match_length, $check_read_pos, $trim_length, $num_unencoded_G, $non_G_mismatch];
...
#....................................................................................................................................................#
#	subroutineCategory: unassigned
#	dependOnSub: checkChunkResults|362, reportAndLogStatus|1368
#	appearInSub: >none
#	primaryAppearInSection: 3_convert|186
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
	...
	print SANITYCHECK join "", (join "\t", ('out_chrom', 'bedStart', 'bedEnd', 'strand', 'rd_seq', 'check_read_seq', 'check_genome_seq', 'cigar', 'softclip_length', 'softclip_seq_rev', 'match_length', 'check_read_pos', 'trim_length', 'num_unencoded_G', 'non_G_mismatch')), "\n";
	...
	foreach my $chunk_num (sort {$a <=> $b} keys %{$chunk_info_hsh_ref}) {
		...
		my ($valid) = &checkChunkResults($chunk_info_hsh_ref, $chunk_num, $file_check_ary_ref);#->362
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 4_finishingTasks|198
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
#	subroutineCategory: general
#	dependOnSub: currentTime|495
#	appearInSub: >none
#	primaryAppearInSection: 2_defineOutDirPath|159, 4_finishingTasks|198
#	secondaryAppearInSection: >none
#	input: $StartOrFinishMessage
#	output: none
#	toCall: &printStartOrFinishMessage($StartOrFinishMessage);
#	calledInLine: 173, 202
#....................................................................................................................................................#
...
#....................................................................................................................................................#
#	subroutineCategory: unassigned
#	dependOnSub: checkChunkResults|362, convertBamToCTSS|385, getChunkFasta|802, pileupCtss|865, reportAndLogStatus|1368
#	appearInSub: >none
#	primaryAppearInSection: 3_convert|186
#	secondaryAppearInSection: >none
#	input: $TSS_mode, $bamPath, $bedtools_bin, $chrom_name_size_hsh_ref, $chunk_info_hsh_ref, $exclude_flag, $genome_fasta_path, $include_flag, $max_non_G_mismatch, $max_softclip_length, $max_thread, $min_MAPQ, $min_match_length, $out_chrom_size_hsh_ref, $samtools_bin, $sanity_check_toggle, $unencoded_G_upstrm_nt
#	output: 
#	toCall: &processPerChunk($bamPath, $bedtools_bin, $samtools_bin, $max_thread, $chunk_info_hsh_ref, $chrom_name_size_hsh_ref, $exclude_flag, $include_flag, $min_MAPQ, $genome_fasta_path, $out_chrom_size_hsh_ref, $sanity_check_toggle, $min_match_length, $max_non_G_mismatch, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt);
#	calledInLine: 192
#....................................................................................................................................................#
	my ($bamPath, $bedtools_bin, $samtools_bin, $max_thread, $chunk_info_hsh_ref, $chrom_name_size_hsh_ref, $exclude_flag, $include_flag, $min_MAPQ, $genome_fasta_path, $out_chrom_size_hsh_ref, $sanity_check_toggle, $min_match_length, $max_non_G_mismatch, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt) = @_;
			...
			sub {
				...
				foreach my $chunk_num (keys %{$itemForThrHsh_ref}) {
					...
					if ($valid eq 'yes') {
						...
						my ($chunk_seq_str, $chunk_fasta_offset_pos) = &getChunkFasta($chunk_fasta_path, $out_chrom, $chunk_startPos, $chunk_endPos, $genome_fasta_path, $size, $samtools_bin);#->802
						&convertBamToCTSS($bamPath, $chrom_name_size_hsh_ref, $exclude_flag, $include_flag, $bedtools_bin, $samtools_bin, $min_MAPQ, $chunk_long_ctss_bed_path, $chunk_unsort_long_ctss_bed_path, $region_str, $chunk_log_path, $chunk_startPos, $chunk_endPos, $chunk_fasta_path, $chunk_seq_str, $chunk_fasta_offset_pos, $chunk_sanity_check_path, $sanity_check_toggle, $min_match_length, $max_non_G_mismatch, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt);#->385
						&pileupCtss($chunk_long_ctss_bed_path, $chunk_collapse_ctss_bed_path, $chunk_ung_collapse_ctss_bed_path, $chunk_num);#->865
					}
					$num_proc++;
...
#	secondaryAppearInSection: >none
#	input: none
#	output: $TSS_mode, $bamPath, $exclude_flag, $genome, $include_flag, $max_softclip_length, $max_thread, $min_MAPQ, $outDir, $outputPrefix, $overwrite, $unencoded_G_upstrm_nt
#	toCall: my ($bamPath, $exclude_flag, $include_flag, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt, $min_MAPQ, $max_thread, $genome, $outputPrefix, $outDir, $overwrite) = &readParameters();
#	calledInLine: 140
#....................................................................................................................................................#
	
	my ($bamPath, $exclude_flag, $include_flag, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt, $min_MAPQ, $max_thread, $genome, $outputPrefix, $outDir, $overwrite);
	
	$min_MAPQ = 0;
	$exclude_flag = '4,8,128,256,512,1024,2048';
	$unencoded_G_upstrm_nt = 2;
	$max_thread = 5;
	$max_softclip_length = 10;
	$overwrite = 'no';
	$TSS_mode='first_match';
	...
	return($bamPath, $exclude_flag, $include_flag, $TSS_mode, $max_softclip_length, $unencoded_G_upstrm_nt, $min_MAPQ, $max_thread, $genome, $outputPrefix, $outDir, $overwrite);
...
#	dependOnSub: currentTime|495
#	appearInSub: checkAllExecutable|238, checkChunkResults|362, convertBamToCTSS|385, generateChromNameConversionHsh|513, generateChunkInfoHsh|583, getCTSS|659, getChunkFasta|802, pileupCtss|865, poolChunkResults|945, processPerChunk|1217
#	primaryAppearInSection: 2_defineOutDirPath|159
#	secondaryAppearInSection: 2_defineOutDirPath|159, 3_convert|186
#	input: $lineEnd, $message, $numTrailingSpace
#	output: 
#	toCall: &reportAndLogStatus($message, $numTrailingSpace, $lineEnd);
#	calledInLine: 174, 175, 176, 177, 178, 179, 261, 266, 275, 284, 293, 302, 311, 320, 329, 378, 400, 459, 489, 528, 538, 565, 607, 793, 796, 815, 916, 980, 1049, 1052, 1251, 1258, 1261, 1279, 1296, 1300
#....................................................................................................................................................#
	...
	print "[".&currentTime()."] ".$message.$trailingSpaces.$lineEnd;#->495
	print $tmplog_fh "[".&currentTime()."] ".$message.$lineEnd if $lineEnd ne "\r";#->495
